...
class FishDataset(Dataset):
    def __init__(self, data_path, img_dir, transform=None, feature_sets=["coords"], depth_model=None):
    ...
    def __getitem__(self, idx):
        ...
        return image, torch.tensor(aux, dtype=torch.float32), torch.tensor(length, dtype=torch.float32), img_name

def train_cnn(dataset_name, epochs=10, batch_size=16, lr=1e-4, feature_sets=["coords"], depth_model=None):
    ...
    train_set = FishDataset(f"{base_dir}/processed_train.csv", img_dir, transform, feature_sets, depth_model)
    val_set = FishDataset(f"{base_dir}/processed_val.csv", img_dir, transform, feature_sets, depth_model)
    test_set = FishDataset(f"{base_dir}/processed_test.csv", img_dir, transform, feature_sets, depth_model)
    ...
    for epoch in range(epochs):
        ...
        print(f"Epoch {epoch+1}/{epochs} - Train Loss: {train_loss/len(train_set):.4f} - Val MAPE: {val_mape:.4f}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--feature-set", type=str, nargs='+', choices=["coords", "scaled", "eye"], default=["coords"])
    parser.add_argument("--depth-model", type=str, choices=["v3", "v2", "pro", "all"], default=None)
    args = parser.parse_args()
    
    train_cnn(args.dataset, epochs=args.epochs, feature_sets=args.feature_set, depth_model=args.depth_model)
