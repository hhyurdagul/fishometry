...
def train_mlp(dataset_name, epochs=100, batch_size=16, lr=1e-3, feature_sets=["coords"], depth_model=None):
    ...
    # Load Data
    try:
        X_train, y_train, names_train = load_data(f"{base_dir}/processed_train.csv", feature_sets, depth_model)
        X_val, y_val, names_val = load_data(f"{base_dir}/processed_val.csv", feature_sets, depth_model)
        X_test, y_test, names_test = load_data(f"{base_dir}/processed_test.csv", feature_sets, depth_model)
    ...
    if len(X_val) > 0: X_val = scaler.transform(X_val)
    if len(X_test) > 0: X_test = scaler.transform(X_test)
...
def main():
    ...
    train_mlp(args.dataset, epochs=args.epochs, feature_sets=args.feature_set, depth_model=args.depth_model)
