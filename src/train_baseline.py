...
def train_baseline(train_path, val_path, test_path, dataset_name):
    ...
    if has_fish_type:
        ...
        means = df_train.group_by("fish_type").agg(pl.col("length").mean().alias("mean_length"))
        # Convert to dictionary for easy lookup
        # means_dict = {row['fish_type']: row['mean_length'] for row in means.to_dicts()}
    ...
    for split_name, path in [("train", train_path), ("val", val_path), ("test", test_path)]:
        ...
        if model_stats["type"] == "global":
            ...
            if "fish_type" not in df.columns:
                print(f"Warning: fish_type missing in {split_name} but model is grouped. Using global mean.")
                ...
                df_with_pred = df_with_pred.with_columns(
                    pl.col("mean_length").fill_null(model_stats["global_mean"]).alias("pred_length")
                )
