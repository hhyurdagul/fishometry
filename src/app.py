...
def main():
    ...
    mode = st.sidebar.radio(
        "Mode",
        ["Data Explorer", "Error Analysis", "Model Comparison", "Correlation Analysis"],
    )
...
def render_analysis(dataset, df_meta, all_image_names, depth_model):
    ...
    sort_by = st.radio(
        "Sort By",
        ["Absolute Error (Descending)", "Percentage Error (Descending)"],
        horizontal=True,
    )
...
def render_correlation(dataset, df_meta):
    ...
    default_features = [
        f
        for f in ["Fish_w", "Fish_h", "Fish_w_scaled", "Fish_h_scaled"]
    ...
    heatmap_df = pl.DataFrame(corr_data)

    heatmap = (
        alt.Chart(heatmap_df.to_pandas())
        .mark_rect()
        .encode(
            x=alt.X("Feature 1:N", title=None, axis=alt.Axis(labelAngle=-45)),
            y=alt.Y("Feature 2:N", title=None),
            color=alt.Color(
                "Correlation:Q",
            ...
            tooltip=[
                "Feature 1",
                "Feature 2",
                alt.Tooltip("Correlation", format=".3f"),
            ],
        )
        .properties(width=500, height=500, title="Feature Correlation Matrix")
    )

    # Add text labels - use calculated field for text color
    # White text for strong correlations (|r| > 0.5), black otherwise
    heatmap_df_with_color = heatmap_df.with_columns(
        pl.when((pl.col("Correlation") > 0.5) | (pl.col("Correlation") < -0.5))
        .then(pl.lit("white"))
    ...
    text = (
        alt.Chart(heatmap_df_with_color.to_pandas())
    ...
    if color_by != "None" and color_by in scatter_df.columns:
        scatter = (
            ...
            .encode(
                ...
                tooltip=["name", x_feature, "length", color_by]
                if "name" in scatter_df.columns
                else [x_feature, "length", color_by],
            )
        ...
        scatter = (
            ...
            .encode(
                ...
                tooltip=["name", x_feature, "length"]
                if "name" in scatter_df.columns
                else [x_feature, "length"],
            )
            .properties(width=700, height=400, title=f"{x_feature} vs Length")
            .interactive()
        )
